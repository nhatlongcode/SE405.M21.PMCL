...
namespace SelfBalancedTree
{
    ...
    public class AVLTree<T>
    {
        ...
        {
            return this.comparer.Compare(this.Search(this.Root, arg).Data, arg) == 0;
        }

        public T LowerBound(T arg)
        {
            var node = this.Search(this.Root, arg);
            if (node != null)
                return node.Data;
            else
                return default;
        ...
        public T UpperBound(T arg)
        {
            var node = this.Search(this.Root, arg, false);
            if (node != null)
                return node.Data;
            else
                return default;
        ...
        private Node<T> Search(Node<T> subtree, T data, bool IsLowerBound = true)
        {
            Node<T> result = null;
            bool IsMoveRight = false;
            if (subtree != null)
            {
                int cmp = this.comparer.Compare(data, subtree.Data);
                if (cmp < 0)
                {
                    result = this.Search(subtree.Left, data, IsLowerBound);
                }
                else if (cmp > 0)
                {
                    result = this.Search(subtree.Right, data, IsLowerBound);
                    IsMoveRight = true;
                }
                else
                {
